...
# CSS: anchor saved-case badge to LOAD button and size correctly for 1/2/3 digits.
...
# App snapshot: store mDays explicitly; tDays already exists.
p=Path('js/app.js'); s=p.read_text(encoding='utf-8')
s=s.replace('        tDays,\n        tDate,', '        mDays,\n        tDays,\n        tDate,', 1)
p.write_text(s,encoding='utf-8')

# Export copy: strict INPUT / OUTPUT only.
p=Path('js/export.js'); s=p.read_text(encoding='utf-8')
start=s.find('function copyResults() {')
if start<0: raise SystemExit('copyResults not found')
# last function in current file; replace through end safely.
prefix=s[:start]
new_fn=r'''function copyResults() {
    ...
    const fractionText = (ratio) => {
        ...
        const found = known.find(([v]) => Math.abs(v-ratio) < 1e-9);
    ...
    const outputLines = [
        ...
        `PROPOZABILĂ DUPĂ ZILE MUNCITE: ${calc.life ? '—' : (calc.workDaysResult || fmtDate(calc.tDate) || '—')}`,
    ...
    if (navigator.clipboard?.writeText) {
        navigator.clipboard.writeText(text)
            .then(() => alert('Datele INPUT/OUTPUT au fost copiate.'))
...
'''
p.write_text(prefix+new_fn+'\n',encoding='utf-8')

# Badge display: cap extreme values but preserve exact counts up to 999.
...
# Tests: cache version and output contract basics.
