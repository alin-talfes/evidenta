...
for(const file of ['js/contopiri-core.js','ai/core.js','ai/safety.js','ai/real-doc-deductions.js','ai/real-doc-hardening.js','ai/beta-lot2-hardening.js']) vm.runInContext(read(file),ctx,{filename:file});
const analyze=ctx.AIDocumentSafety.analyze;
const json=v=>JSON.parse(JSON.stringify(v));

// 1. Două mandate independente în același PDF: fail-closed, fără combinarea datelor.
...
// 2. Concurs explicit în listă: 6 luni + 6 luni + 7 ani => 7 ani 4 luni.
...
// 3. Pluralitate intermediară / contopire explicită + documente auxiliare: auxiliarele nu contaminează startDate.
...
assert.equal(a3.startDate,'','Data arestării din fișa urmăritului nu trebuie folosită drept început al executării MEPI.');
assert.equal(a3.ignoredAuxiliaryPages.length,3);
assert.ok(a3.deductions.some(x=>x.type==='retention24h'&&x.start==='12.04.2021'));

// 4. „La zi” cu OCR greșit pe anul final: data mandatului prevalează și rândul cere verificare.
...
// 5. Outlier OCR 72 ani nu poate deveni valoare silențioasă; finala explicită repetată rămâne 2 ani 6 luni.
...
// 6. Contopire inline: 3 ani + 2 ani + 2 ani => 4 ani 4 luni.
