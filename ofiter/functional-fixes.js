(()=>{
  ...
  renderLegislation=function(){const query=normalizeSearch($("#legislation-search")?.value||""),actFilter=$("#legislation-act")?.value||"all",catalog=laws.map(law=>legislationActs.find(act=>act.id===law.id)||{...law,status:law.id==="omj2188"?"sinteza":"link",articles:[]}),acts=catalog.filter(act=>actFilter==="all"||act.id===actFilter),host=$("#legislation-content");if(!host)return;host.innerHTML=acts.map(act=>{const articles=act.articles.filter(article=>{const subsections=(article.subsections||[]).flatMap(section=>[section.intro,...(section.items||[])]),haystack=normalizeSearch([article.number,article.heading,article.intro,...(article.items||[]),...subsections].join(" "));return !query||haystack.includes(query)}),inSynthesis=act.status==="sinteza";return `<section class="legal-act"><header><div><p class="eyebrow">DELIMITARE DIN BIBLIOGRAFIE</p><h2>${act.title}</h2><p class="legal-scope">${act.scope}</p></div><div class="legal-act-actions"><span class="integration-status ${act.status==="integrat"||inSynthesis?"done":"pending"}">${act.status==="integrat"?"✓ Text integrat":inSynthesis?"✓ Text explicat în Sinteză":"Consultă sursa oficială"}</span><a class="official-link" href="${act.url}" target="_blank" rel="noopener">Deschide forma oficială consolidată ↗</a></div></header>${act.articles.length?(articles.length?articles.map(renderLegalArticle).join(""):'<div class="empty">Niciun articol nu corespunde căutării.</div>'):`<div class="legal-placeholder">${inSynthesis?"Conținutul explicat al OMJ nr. 2188/C/2022 este disponibil în modulul Sinteză. Pentru verificarea integrală, deschide forma oficială consolidată.":"Pentru conținutul integral și actualizat, folosește forma consolidată oficială."}</div>`}</section>`}).join("")};
